...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    {
        let str:String = String(describing: url)
        if let index = str.index(of: "=")
        {
            codeString = String(str[index...])
            codeString.remove(at: codeString.startIndex)
        }
        AppDelegate.viewController!.schoolApiController?.getToken(code: codeString)
        return true
    }
